...
1. Name the message of the chart before choosing its form.
2. Choose chart type from the data relationship (comparison, trend, distribution, part-to-whole).
3. Encode honestly: zero-based bars, unbroken axes, colorblind-safe palettes, no color-only meaning.
4. Label directly where possible; title with the takeaway, not the dataset name.
5. Review against accessibility and the audience's decision context.
