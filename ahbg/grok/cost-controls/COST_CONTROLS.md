...
Corpus run: `corpus-run/calibration-family-1.0.0-proposal-1`
...
- `runtime_burden_observables` — **BLOCKED**. tokens, latency, retries, and tool_calls are hmmm on every telemetry row. No runtime burden series exists to map C_lambda onto.
...
- `voluntary_disengagement_capacity_preserving` — **UNRESOLVED**. voluntary_disengagement still relocates three times. Resource telemetry remains hmmm, so capacity preservation is not measured.
